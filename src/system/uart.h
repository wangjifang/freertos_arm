...
#include "fifo.h"
#include "osel_arch.h"
#define UART_SIZE                       (254u)
#define UART_SD_LEN_MAX             2
#define UART_LD_LEN_MAX             2
...
typedef struct _uart_reg_t
{
    struct
    {
        uint8_t len;
        uint8_t pos;
        uint8_t data[UART_SD_LEN_MAX];
    } sd; //start delimiter
...
typedef struct
{
     ...
     */
    void ( *IrqNotify )( void *obj, UartNotifyId_t id );
    TimerHandle_t start_timer;
    TimerCallbackFunction_t timeout_callback;
    uart_reg_t uart_reg;
}Uart_t;
