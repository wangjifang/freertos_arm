...
void USART1_IRQHandler( void )
{
    ...
    {    
	if( IsFifoEmpty( &Uart[UART_1].FifoTx ) == FALSE )
	{
	    data = FifoPop( &Uart[UART_1].FifoTx );
	...
	if( Uart[UART_1].IrqNotify != NULL )
	{
	    Uart[UART_1].IrqNotify((void *) &Uart[UART_1], UART_NOTIFY_TX );
	}
    }
	...
	if( IsFifoFull( &Uart[UART_1].FifoRx ) == FALSE )
	{
	    // Read one byte from the receive data register
	    FifoPush( &Uart[UART_1].FifoRx, data );
	}
	if( Uart[UART_1].IrqNotify != NULL )
	{
	    Uart[UART_1].IrqNotify((void *) &Uart[UART_1], UART_NOTIFY_RX );
	}
    }
}
